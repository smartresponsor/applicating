...
class Product implements \App\Component\Product\Interface\Product\ProductInterface
{
    ...
    /** @return Collection<int, ProductI18n> */ public function translations(): Collection { return $this->translations; }
    public function addTranslation(ProductI18n $i18n): void { $this->translations->add($i18n); $this->touch(); }
    public function changePrice(Money $newPrice): void { $this->price = $newPrice; $this->touch(); }
    public function adjustStock(int $delta): void { $new=$this->stock+$delta; if($new<0) throw new \DomainException('Stock cannot be negative'); $this->stock=$new; $this->touch(); }
