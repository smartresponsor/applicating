...
#[ORM\Entity(repositoryClass: 'App\\Component\\Product\\Repository\\ProductRepository')]
...
class Product
{
    ...
    public function rename(string $locale, string $name): void
    {
        $t = $this->getOrFail($locale);
        $t->rename($name);
        $this->touch();
    }

    public function changeSlug(string $locale, string $slug): void
    {
        $t = $this->getOrFail($locale);
        $t->changeSlug($slug);
        $this->touch();
    ...
    public function changeDescription(string $locale, ?string $description): void
    {
        $t = $this->getOrFail($locale);
    ...
    private function getOrFail(string $locale): ProductI18n
    {
        foreach ($this->translations as $t) {
            if ($t->locale() === $locale) { return $t; }
        }
        throw new \DomainException('Translation not found for ' . $locale);
    }
}
