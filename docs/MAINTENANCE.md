...
- `qa.yml` — canonical full QA path on pull requests, pushes to `master` and manual dispatch
- `ci-precheck.yml` — verifies the active operational contour exists
- `ci-smoke.yml` — runs runtime, fixture and container smokes on pull requests and pushes to `master`
