...
$out = $reportDir . '/applicating-route-inventory-report.json';
...
if (is_dir($routeDir)) {
    foreach (new DirectoryIterator($routeDir) as $file) {
        ...
        $routes[] = [
            'type' => 'yaml',
            'file' => 'config/routes/' . $file->getFilename(),
        ];
    }
}
...
echo sprintf(
    "[ApplicatingRouteInventoryReport] %d route inventory rows written to %s\n",
    count($routes),
